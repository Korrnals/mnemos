...
flowchart TD
    ...
    F1["Ф1 · МИНИМАЛЬНЫЙ ЭКСПЕРИМЕНТ (S5)<br/>A: B0+type_boost · B: эмуляция task-линзы<br/>C: честный tag-фильтр · предрегистрация ДО"]
...
- [ ] **Ф0 — композиция без схемы** (первая пользовательская ценность:
  ...
  - [ ] опциональный `task`-параметр `assemble_context` (только хвост;
        префиксы не трогать — инвариант #280).
...
### Инварианты эпика (нарушение любого = стоп-волна)

Байт-стабильность префиксов (#280; новые входы сортировки — после
id-tiebreak, веса целочисленные); lanes default-off (вердикт E3 не
пересматривается); awareness tail/never-pinnable/born-no-federate +
неприкосновенность `awr:`-курсоров; worst-link транзитивность
no-federate/карантина; линза только сужает; повторный секрет-скан
кросс-контекстных сборок; born-quarantine ingested-документов; #248 =
блокер проекций-пинов; value-заявления только через S5 PASS (ADR-0026).
...
| P1 | [#308](https://github.com/Korrnals/mnemos/issues/308) | Эпик многоконтекстной памяти (АрхКом 2026-09-14, accept-staged, ADR-0027): Ф0 композиция без схемы → Ф1 эксперимент-гейт (S5, руки A/B/C) → Ф2 task-примитив за флагом → Ф3 docs-as-memory | план §4b; Ф0 — следующая волна после #280 Phase-1 |
