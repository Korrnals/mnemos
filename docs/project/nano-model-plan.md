...
**Рефайнер (этап NM-3):** **Qwen3.5-0.8B** (Apache-2.0, 2026-02, 256K контекст, 201 язык, свежее Qwen3-0.6B на 8 месяцев; оговорка: это VLM — при текст-only vision-башня игнорируется; RU-поддержка подтверждается смоук-тестом суммаризации при первом касании) — основной; Qwen3-0.6B int4 — запасной; Qwen2.5-0.5B — резерв. SmolLM2/SmolLM3-класс — EN-only, отсечён. LFM2.5-350M (июль 2026, RU в 15 языках) — свеж, но base-модель под свой SFT и лицензионно менее удобен.
