...
6. **Conflict-store mass:** per scenario block, 2–3 checkpoints + 1 evidence row (type-1 only) + 6 noise rows; type-1 peers also write lexically overlapping goals — hint contact is uniform across the stratum, and the type split fixes WHERE the claim is discoverable, not whether a peer goal exists.
