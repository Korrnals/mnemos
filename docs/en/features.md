...
| **Flexible memory** | Hybrid search (full-text + vector, rank fusion), the [tag contract](user/tag-contract.md), memory scoped per agent and per project, a [context filter](user/context-filter.md) with content profiles (code / prose / docs), and CCR compression — marker in context, original in memory, 70–90% token savings |
