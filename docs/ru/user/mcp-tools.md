...
- **Валидация на границе** — пустые `content`/`project`/`agent`, пустые опциональные строки, нарушение контракта тегов (strict-режим) или несуществующий `supersedes` возвращают `{"error": …}` (REST-близнец отвечает 422).
