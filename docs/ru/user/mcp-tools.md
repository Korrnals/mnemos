...
Любая непройденная проверка возвращает refused-форму с `reason="marker validation failed: <проверка>: <деталь>"` и **без контента** (fail-closed). Строки, сохранённые без идентичности эмитента (легаси-миграции, сжатие без идентичности), неверифицируемы и отклоняются с отдельной причиной `unverifiable legacy marker`. Обычные запросы только с хешем не затрагиваются ни в одном режиме. Отказ валидации не инкрементирует `retrieval_count`.
