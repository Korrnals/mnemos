...
— без `original`/`snippets` (fail-closed), `retrieval_count` не инкрементируется. Строки без идентичности эмитента отклоняются с отдельной причиной `unverifiable legacy marker`. Обычные запросы только с хешем не затрагиваются. Остаточный риск: посев внутри проекта доверенным вызывающим compress принят (реестр остаточных рисков ADR-0018, single-operator).
